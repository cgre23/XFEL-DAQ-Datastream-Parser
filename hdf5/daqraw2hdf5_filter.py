        ...
        if inttype:
            ...
            if stats['type'] == 'IMAGE':
                ...
                if darray.ndim == 1:
                    ...
                    if (groupname.find("DIAG/BPM/BPM") != -1 or groupname.find("SDIAG/BCM/BCM") != -1 or groupname.find("TOROID/TORA") != -1) and dest != []:
                        filter_indices = [int(index / 2) for index in indexout]
                        #print('Filtering indices:', filter_indices)
                        data = group.create_dataset(
                        subchname, dtype="float32", data=darray[:, filter_indices], compression='gzip', compression_opts=compression, chunks=True, maxshape=(None, None,))
                        #data = group.create_dataset(
