...
daqeventtypes = {
    ...
    '21':'BEAM_ENERGY_BYPASS',# all electron beam ENERGY related event (in BYPASS)
...
'6':'BLOCK_BPM',                     # BPM with X and Y channels (2 ADC channels)
'7':'BLOCK_BPM_Z',                # BPM (Zeuthen Cavity, X, Y , I, Q -  4 ADC channels)
...
'31':'TTF2_UPD_BLOCK_DONT_USE_31',       # shouldn't be used beacuse 0x80000000 us used as the indicator for
...
'38':'BLOCK_BPM_2010',               # BPM with X and Y channels (2 ADC channels)
'39':'BLOCK_BPM_Z_2010',          # BPM (Zeuthen Cavity, X, Y , I, Q -  4 ADC channels)
...
daqdescrfile =['/daq/ttf2/adm/daq_channel_descriptions.xml','/daq/xfel/adm/daq_channel_descriptions.xml']
...
def get_channel_file_list(xmlfile, linac, localmode, onlywithevents, debug):
    if not xmlfile or not linac:
        print('get_channel_file_list: please check your parameters tmpxmlfile:', xmlfile, 'linac', linac, 'localmode', localmode)
        print('get_channel_file_list: exiting ...')
        sys.exit(-1)

    try:
        err = pydaq.connect(xml=xmlfile, linac=linac, local=localmode, mode ='list', filelist=True) 
        ...
        for entry in err:
            if  'files' in entry:
                ...
                if not onlywithevents or (onlywithevents and (entry['events'] > 0)):
                    ...
                    if debug:
                        print('[%d] %s events:%d subchannels:%d'%(i, entry['daqname'], entry['events'], len(entry['miscellaneous'])))
                        for sub in entry['miscellaneous']:
                            print("\tindex:%d name:%s units:%s descr:%s"% (sub['dim'], sub['name'], sub['units'], sub['description']))
                i += 1
